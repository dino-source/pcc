...
stop = 5
sliced_list = digits[:stop]  # from the very beginning through stop-1
print(f"digits[:5] starts slice at the beginning of the list: {sliced_list}\n")


start = 2
sliced_list = digits[start:]  # from 'start' index through the end of the list
...
sliced_list = digits[:except_n_last_items]  # everything but the last two items
print(f"digits[:-2] gets everything but the last two items: {sliced_list}\n")
...
print(
    "digits[0:7:2] gets items from the very beginning through 7th item,\n\t"
...
sliced_list = digits[start:stop:step]  # start through not past stop, by step
print(
    "digits[1:8:3] gets items from the 2nd item (so item[1]) through 8th item,"
    + "\n\t"
...
start = -3
step = -1
sliced_list = digits[start::step]  # everything but the last 2 items, reversed
print("digits[-3::-1] gets everything except the two last items, " + f"reversed: {sliced_list}\n")
